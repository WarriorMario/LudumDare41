...
SGNode* Scene::LoadOBJ( const char* file, const float _Scale )
{
  printf("Start loading obj %s.\n", file);
	...
	FILE* f = fopen( file, "r" );
  string serializedFileName = file;
  serializedFileName += ".vbvh";
  bool bakedBVH = false;
  FILE* bakedBVHFile = fopen( serializedFileName.c_str(), "rb");
  vector<DynamicBVH*> bvhs;
  if ( bakedBVHFile != nullptr )
  {
    int num;
    fread( &num, sizeof( num ), 1, bakedBVHFile );
    bakedBVH = true;
    for ( int i = 0; i < num; ++i )
    {
      // Load into vector
      bvhs.push_back( new DynamicBVH( bakedBVHFile ) );
    }
  }
	ExtractPath( file );
	if (f) // obj file loader: converts indexed obj file into indexed multi-mesh
	{
      ...
      if ( bakedBVH == false )
      {
        printf("Creating dynamic bvh with index %i. \n", bvhList.size());
        DynamicBVH* bvh = new DynamicBVH( current );
  ...
  {
    // Serialize
    FILE* serializedFile = fopen( serializedFileName.c_str(), "wb" );
    int num = bvhs.size();
    fwrite( &num, sizeof( num ), 1, serializedFile );
    for ( int i = 0; i < num; ++i )
    {
      bvhs[i]->Serialize( serializedFile );
    }
  }
